...
export default function VacationShield({ setTab }) {
  ...
  // A named trip length, so the common case does not need a date picker.
  function quickTrip(nights) {
    const now = new Date();
    const end = new Date(now.getTime() + nights * 86400000);
    const services = PAUSABLE.filter((p) => p.id !== 'fiber' || fiberActive);
    dispatch({
      ...
      vacation: {
        ...
        days: nights,
        rental: 0,
        saving: Math.round(services.reduce((s, p) => s + p.dailyRate * nights, 0)),
        services: services.map((p) => p.name),
      },
    ...
    return (
      <>
        <div className="vac-head">
          ...
          <div>
            <span className="eyebrow">Vacation Shield</span>
            <h2>Paused, not cancelled</h2>
          </div>
        </div>

        <div className="vac-booked">
          <span className="vac-tick" aria-hidden="true">✈</span>
          <strong>Vacation Shield Activated</strong>
          <span className="vac-headline">₹0 rental · {booked.days} days</span>
          <span>
            {pretty(booked.from)} to {pretty(booked.to)} · about ₹
            {booked.saving.toLocaleString('en-IN')} not billed
          </span>
        </div>
    ...
    <>
      <div className="vac-head">
        ...
        <div>
          ...
          <h2>Introducing Vacation Shield</h2>
        </div>
      </div>

      {/* The film needs a single tap and a 2 second screen, so the default
          trip is one button. The date picker underneath is still there for
          anyone who wants a different window. */}
      <button className="vac-oneTap" onClick={() => quickTrip(10)}>
        <span>
          <b>Activate for 10 days</b>
          <small>Standard trip · starts today · ₹0 rental while you are away</small>
        </span>
      ...
      <div className="tile">
        ...
        {overLimit && (
          <p className="vac-days vac-error">
            Vacation Shield covers up to {MAX_DAYS} days a year. Shorten the break or split it.
          </p>
        )}
      </div>
