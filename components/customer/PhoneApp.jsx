...
import VacationShield from './VacationShield';
import SafeGuard, { SafeGuardBanner } from './SafeGuard';
import NbaPush from './NbaPush';
  ...
  return (
    <div className="phone">
      ...
      <div className="phone-body">
        ...
        {!loading && tab === 'vacation' && <VacationShield setTab={setTab} />}
        {!loading && tab === 'safeguard' && <SafeGuard setTab={setTab} />}
        {!loading && tab === 'kyc' && <KycFlow setTab={setTab} />}
        {!loading && tab === 'onboard' && <OnboardTab setTab={setTab} />}
        {!loading && tab === 'bank' && <BankTab setTab={setTab} />}
      ...
      <nav className="tabbar">
        {TABS.map((t) => (
          <button
            key={t.id}
            data-on={tab === t.id || (tab === 'onboard' && t.id === 'offers') || (tab === 'kyc' && t.id === 'home') || (tab === 'vacation' && t.id === 'bank') || (tab === 'safeguard' && t.id === 'home')}
            onClick={() => setTab(t.id)}
          >
            <b>{t.icon}</b>
            {t.label}
  ...
  return (
    <>
      ...
      <div className="usage-panel">
        ...
        <p>
          You ran out on day 24 of 28. That is five times this quarter, and you have bought 5
          top-ups worth ₹605 above what the pack already covers.
        </p>
      </div>
...
function BankTab({ setTab }) {
  ...
  return (
    <>
      ...
      {/* Small by design. It lives here because this is the screen people open
          when they are thinking about what they are paying for. */}
      <button className="vac-teaser" onClick={() => setTab('vacation')}>
        <span className="vac-teaser-icon" aria-hidden="true">✈</span>
        <span className="vac-teaser-body">
          <b>{booked ? 'Vacation Shield is on' : 'Going on vacation?'}</b>
          <small>
            {booked
              ? `Paused until ${new Date(booked.to).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}. Tap to change.`
              : 'Introducing Vacation Shield. Pause your broadband and pack while you are away.'}
          </small>
        </span>
        <span className="vac-teaser-go" aria-hidden="true">›</span>
      </button>
