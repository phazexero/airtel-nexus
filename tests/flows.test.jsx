...
describe('console shell', () => {
  it('loads the working set and lands on the first conversation', async () => {
    ...
    await ready();
    expect(await screen.findByRole('heading', { name: 'Ananya Sen' })).toBeInTheDocument();
...
describe('campaign studio', () => {
  it('requires an area, then builds a brief', async () => {
    ...
    const build = await screen.findByRole('button', { name: 'Build the campaign' });
    expect(build).toBeDisabled();

    await user.click(screen.getByRole('button', { name: 'Select Behala' }));
  ...
  it('launches into a cluster once', async () => {
    ...
    console_(<FeaturePane title="Campaign studio" lede="x"><CampaignStudio /></FeaturePane>);
    await ready();
    await user.click(screen.getByRole('button', { name: 'Select Jadavpur' }));
    await user.click(screen.getByRole('button', { name: 'Build the campaign' }));
...
describe('other surfaces', () => {
  ...
  it('walks every tab of the customer app', async () => {
    ...
    await screen.findByRole('heading', { name: 'Ananya Sen', level: 1 }, { timeout: 4000 });
    ...
    expect(screen.getByRole('heading', { name: 'Ananya Sen', level: 1 })).toBeInTheDocument();
  });
});
