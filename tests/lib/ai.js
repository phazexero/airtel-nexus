...
// Opportunity band, used by the map's second colour mode. Low penetration and
// high value is the combination worth spending against.
export function opportunityBand(locality) {
  const loc = typeof locality === 'string' ? localityById(locality) : locality;
  const headroom = (100 - loc.fiberPenetration) / 100;
  const value = Math.min(loc.arpu / 400, 1);
  const churn = loc.churnRisk === 'high' ? 1 : loc.churnRisk === 'medium' ? 0.7 : 0.45;
  const score = Math.round((headroom * 0.45 + value * 0.35 + churn * 0.2) * 100);
  const band = score >= 72 ? 3 : score >= 62 ? 2 : score >= 52 ? 1 : 0;
  return { score, band, label: ['Watch', 'Worth a look', 'Strong', 'Priority'][band] };
}
