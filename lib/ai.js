import { PRODUCTS, localityById } from './data';
...
const OPENERS = {
  ...
  analytical: (c) =>
    `Lead with the arithmetic, not the adjective: "You have spent ₹484 above your pack this quarter. Let me show you what that money buys on a different structure."`,
  skeptical: () =>
...
function pickAction(customer) {
  ...
  if (customer.id === 'C-88214') {
    return {
      ...
      stage: 'Prepaid to Black',
      why: [
        '₹484 spent above pack value in 60 days across 4 top-ups',
      ...
      risk: 'She left at the price screen, so the price is the objection. Open with the ₹484 she already spent, not with ₹1,099.',
    };
  }
...
export function nextBestAction(customer) {
  ...
  // return decision;
  // ========================================================================
  const base = pickAction(customer);
  const p = customer.profile;
...
export function buildCampaign(localityId, objective = 'upsell', budget = 250000) {
  // === AI INTEGRATION POINT ===============================================
  // Same shape as above. Send { task: 'campaign', locality, objective, budget }
  // to /api/ai and return the decision object unchanged.
  // ========================================================================
  const loc = localityById(localityId);
  const product = PRODUCTS[loc.product];
