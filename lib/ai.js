...
// ===========================================================================
//  ALTAI SUGGESTION BOX
//
//  A campaign brief gives one answer. A distributor standing in front of a
//  cluster usually has three or four legitimate plays available and is choosing
//  between them, so handing over a single option reads as a black box.
//
//  These are directions, not a ranking dressed up as a choice. Each names the
//  thrust, two concrete moves, and the trade it makes, so the person picking
//  can defend the pick. The first is the one the rules engine would have taken
//  on its own, which keeps the default honest.
// ===========================================================================
...
const DIRECTIONS = {
  fd: [
    {
      id: 'rate-first',
      name: 'Lead with the rate',
      thrust: 'Put the two interest numbers side by side and say nothing else.',
      moves: ['Bilingual leaflet, rate above the fold', 'Assisted onboarding at society gates'],
      tradeoff: 'Highest conversion per contact, but only reaches people already thinking about money.',
    },
    {
      id: 'trust-first',
      name: 'Lead with the insurance',
      thrust: 'Deposit safety before yield, for an audience that has been sold to badly before.',
      moves: ['₹5 lakh cover as the headline', 'Named local officer on every asset'],
      tradeoff: 'Slower to convert, far lower complaint and reversal rate afterwards.',
    },
    {
      id: 'family-proxy',
      name: 'Sell through the children',
      thrust: 'Target the earning generation who manage their parents money remotely.',
      moves: ['In-app prompt on the payer profile, not the holder', 'Remote-open flow with two-name access'],
      tradeoff: 'Doubles the addressable audience, at the cost of a longer decision chain.',
    },
  ],
  black: [
    {
      id: 'bill-shock',
      name: 'Show them their own bill',
      thrust: 'Compare their actual current total against the bundle price, per household.',
      moves: ['In-app card at the moment of payment', 'WhatsApp with a personalised comparison'],
      tradeoff: 'Very hard to argue with, but needs billing data per household to land.',
    },
    {
      id: 'building-capture',
      name: 'Take the building, not the flat',
      thrust: 'Fiber is already lit, so win one address at a time rather than one line at a time.',
      moves: ['Lobby and lift panels in lit buildings', 'Referral credit for existing users in the same block'],
      tradeoff: 'Slower start, then compounds. Best where install lead time is under 48 hours.',
    },
    {
      id: 'one-date',
      name: 'Sell the single date',
      thrust: 'Not the price, the end of four payment dates and four reminders.',
      moves: ['Creative built entirely on the calendar', 'Choose-your-billing-date at signup'],
      tradeoff: 'Weak on price-led buyers, strong on households already paying more than the bundle.',
    },
  ],
  annualPack: [
    {
      id: 'monthly-framing',
      name: 'Price it monthly, sell it yearly',
      thrust: 'Never say ₹3,599. Say ₹300 a month and let the validity do the work.',
      moves: ['Retailer counter kit with the monthly figure', 'Salary-week SMS on the 3rd to 6th'],
      tradeoff: 'Removes the lump-sum objection but needs the retailer to hold the line on framing.',
    },
    {
      id: 'lapse-rescue',
      name: 'Catch them at the lapse',
      thrust: 'Trigger on the day validity expires, when the pain is live rather than remembered.',
      moves: ['Same-day vernacular call on lapse', 'Grace-day offer attached to the recharge'],
      tradeoff: 'Small window and needs event-driven targeting, but the highest intent moment there is.',
    },
    {
      id: 'retailer-led',
      name: 'Pay the counter, not the media',
      thrust: 'This cluster recharges over the counter, so move the incentive to the retailer.',
      moves: ['Per-conversion retailer incentive', 'Counter training on the annual maths'],
      tradeoff: 'Cheapest cost per acquisition here, but the message quality depends on 60 shopkeepers.',
    },
  ],
  merchant: [
    {
      id: 'settlement',
      name: 'Lead with same-day settlement',
      thrust: 'The competitor is already here, so sell the one thing they do not match.',
      moves: ['Settlement timing printed on the standee', 'Feet on street between 11:00 and 15:00'],
      tradeoff: 'Wins switchers rather than first-time adopters, so volume is capped but quality is high.',
    },
    {
      id: 'association',
      name: 'Go through the market association',
      thrust: 'Trader clusters adopt in groups, so win the group and the shops follow.',
      moves: ['Association tie-up with a bulk onboarding day', 'Named settlement contact for the market'],
      tradeoff: 'Slow to start and political, then converts a whole street at once.',
    },
    {
      id: 'cash-cost',
      name: 'Price the cash they handle',
      thrust: 'Make the cost of counting, banking and shrinkage visible, then remove it.',
      moves: ['One-page daily cash cost calculator', 'Free QR with a closing-time SMS summary'],
      tradeoff: 'Needs a longer conversation than a doorstep pitch allows.',
    },
  ],
  fiber: [
    {
      id: 'per-head',
      name: 'Divide it by the flat',
      thrust: 'Lead with the per-person monthly figure, never the connection price.',
      moves: ['Hostel and shared-flat notice boards', 'Peak-hour campus-adjacent digital'],
      tradeoff: 'Converts fast, but the buying unit is a group that can dissolve mid-term.',
    },
    {
      id: 'no-lock',
      name: 'Sell the exit',
      thrust: 'A nine-month residency will not sign a twelve-month contract, so put cancellation in the headline.',
      moves: ['No-lock-in stated above the fold', 'Cancel-from-app in one screen'],
      tradeoff: 'Higher churn by design, offset by a much larger addressable base.',
    },
    {
      id: 'pack-maths',
      name: 'Count their add-on packs back to them',
      thrust: 'They already spend more than fiber costs, in fragments they never total up.',
      moves: ['In-app monthly add-on spend summary', 'Referral credit inside the same building'],
      tradeoff: 'Only works on heavy users, which is most of this cluster but not all of it.',
    },
  ],
  savings: [
    {
      id: 'moment',
      name: 'Catch the post-recharge moment',
      thrust: 'Ask when the payment method is already open and trust is already spent.',
      moves: ['In-app banner immediately after recharge', 'Four-minute open with video KYC'],
      tradeoff: 'Cheapest conversion available, limited to people already transacting.',
    },
    {
      id: 'assisted',
      name: 'Open it for them',
      thrust: 'First accounts complete far better with someone standing alongside.',
      moves: ['Retailer-assisted onboarding', 'Vernacular walkthrough at the counter'],
      tradeoff: 'Higher cost per account, much higher completion rate.',
    },
    {
      id: 'zero-balance',
      name: 'Lead with zero balance',
      thrust: 'Remove the barrier before naming the benefit.',
      moves: ['Zero-balance and insurance in the first line', 'Vernacular SMS at scale'],
      tradeoff: 'Broad reach, weaker balances in the first six months.',
    },
  ],
};

export function marketingDirections(locality) {
  // === AI INTEGRATION POINT ===============================================
  // POST { task: 'directions', locality } to /api/ai. A model is a good fit for
  // phrasing these; it is a poor fit for deciding which cluster gets which
  // product, so leave that upstream.
  // ========================================================================
  const loc = typeof locality === 'string' ? localityById(locality) : locality;
  return DIRECTIONS[loc?.product] ?? DIRECTIONS.savings;
}

// Three lines describing what an area actually holds. Deliberately three: it is
// what fits beside a map without pulling attention off it.
export function areaBrief(locality) {
  const loc = typeof locality === 'string' ? localityById(locality) : locality;
  return [
    `${loc.subscribers.toLocaleString('en-IN')} subscribers · median age ${loc.medianAge} · ₹${loc.arpu} ARPU`,
    `${loc.prepaidShare}% prepaid, ${loc.fiberPenetration}% on fiber, ${loc.dataPerUser} GB per user`,
    `${loc.segment}. ${loc.need}.`,
  ];
}
...
//  GROWTH PROSPECTS
//
//  Three product lines, scored separately per area. One combined "opportunity"
//  number hid the thing that actually matters: an area can be a poor prospect
//  for one line and the best one on the map for another. Jodhpur Park is a
//  weak broadband bet and the strongest deposit bet there is, and a single
//  score averages that difference away.
//
//  Each score is 0 to 100 and built from headroom times value times a
//  willingness proxy, so a cluster that is already saturated scores low however
//  rich it is.
// ===========================================================================
...
export const PRODUCT_LINES = {
  postpaid: {
    ...
    label: 'Postpaid',
    hue: '#e40000',
    ramp: ['#4a1616', '#9c1414', '#d81414', '#ff3b3b'],
    note: 'Prepaid users with the spend to carry a monthly bill',
  },
  bank: {
    id: 'bank',
    label: 'Payments Bank',
    hue: '#e0a30c',
    ramp: ['#4a3a10', '#8a6a10', '#c99a12', '#f5c453'],
    note: 'Idle balances and an age profile that holds deposits',
  },
  broadband: {
    id: 'broadband',
    label: 'Broadband',
    hue: '#2f7fd4',
    ramp: ['#16304a', '#1d548a', '#2470bd', '#4aa3ff'],
    note: 'Heavy data on mobile with no fixed line at home',
  },
};

export const PROSPECT_BANDS = ['Thin', 'Fair', 'Strong', 'Priority'];
...
export function growthProspect(locality, line) {
  const loc = typeof locality === 'string' ? localityById(locality) : locality;
  let score = 0;

  if (line === 'postpaid') {
    // Room to convert, times the ability to carry a bill, times the pain of
    // running out. A cluster already on postpaid has nothing left to win.
    const headroom = loc.prepaidShare / 100;
    const value = Math.min(loc.arpu / 320, 1);
    const pain = Math.min(loc.dataPerUser / 30, 1);
    score = (headroom * 0.45 + value * 0.3 + pain * 0.25) * 100;
  } else if (line === 'bank') {
    // Balances that sit still, in an age profile that holds deposits rather
    // than spending through them.
    const idle = loc.savingsIndex / 100;
    const age = Math.min(Math.max((loc.medianAge - 24) / 36, 0), 1);
    const trust = loc.churnRisk === 'low' ? 1 : loc.churnRisk === 'medium' ? 0.65 : 0.35;
    score = (idle * 0.5 + age * 0.3 + trust * 0.2) * 100;
  } else {
    // Data burn with nowhere to put it. Fiber already in the building is
    // headroom gone, not headroom found.
    const headroom = (100 - loc.fiberPenetration) / 100;
    const burn = Math.min(loc.dataPerUser / 34, 1);
    const density = Math.min(loc.subscribers / 58200, 1);
    score = (headroom * 0.4 + burn * 0.4 + density * 0.2) * 100;
  }

  const value = clamp(score);
  const b = band(value);
  return { line, score: value, band: b, label: PROSPECT_BANDS[b] };
}

export function allProspects(locality) {
  return Object.keys(PRODUCT_LINES).map((line) => growthProspect(locality, line));
}

// The line an area is most worth working, which is what a distributor standing
// in front of it actually wants to know.
export function bestLine(locality) {
  return allProspects(locality).reduce((a, b) => (b.score > a.score ? b : a));
}
